...
class ArticleRetriveUpdateDestroyView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if "banner_image" in self.request.FILES:
            ...
            if (
                ...
                and instance.banner_image.name != "article-default.png"
            ):
        ...
        if "body_image_1" in self.request.FILES:
            if (
                instance.body_image_1
                and instance.body_image_1.name != "article-default.png"
            ):
        ...
        if "body_image_2" in self.request.FILES:
            if (
                instance.body_image_2
                and instance.body_image_2.name != "article-default.png"
            ):
    ...
    def retrieve(self, request, *args, **kwargs):
        try:
            ...
            return Response(
                {"message": "Article does not found"}, status=status.HTTP_404_NOT_FOUND
            )

        viewer_ip = request.META.get("REMOTE_ADDR", None)

        # add article view in ArticleViews model
        ArticleViews.record_view(
