...
class Article(TimeStampModel):
    ...
    description = models.CharField(
        verbose_name=_("Article Description"), max_length=255, blank=True
    )
    body = models.TextField(verbose_name=_("Article Body"))

    banner_image = models.ImageField(
        verbose_name=_("Banner Image"),
        upload_to="ArticleMedia/BannerImages",
        default="/article-default.png",
    )
    body_image_1 = models.ImageField(
        verbose_name=_("Body Image 1"),
        upload_to="ArticleMedia/BodyImage1",
        default="/article-default.png",
    )
    body_image_2 = models.ImageField(
        verbose_name=_("Body Image 2"),
        upload_to="ArticleMedia/BodyImage2",
        default="/article-default.png",
    )
    ...
    def __str__(self):
        return (
            f"{self.author.first_name} {self.author.last_name}'s Article - {self.title}"
        )
...
class ArticleViews(TimeStampModel):
    ...
    article = models.ForeignKey(
        Article,
        verbose_name=_("Article being viewed"),
        on_delete=models.CASCADE,
        related_name="article_views",
    )
    user = models.ForeignKey(
        User,
        verbose_name=_("Article viewd by User"),
        on_delete=models.CASCADE,
        related_name="article_views",
    )
    ...
    def record_view(cls, article, user, viewer_ip):
        article_view, created = cls.objects.get_or_create(
