...
class ArticleSerializer(serializers.ModelSerializer):
    ...
    )  # as estimated_reading_time is property, no need to have any method here

    average_rating = (
        serializers.ReadOnlyField()
    )  # average_rating is not declared as @property hense a method is declared
    ...
    def update(self, instance, validated_data):
        ...
        instance.body_image_1 = validated_data.get(
            "body_image_1", instance.body_image_1
        )
        instance.body_image_2 = validated_data.get(
            "body_image_2", instance.body_image_2
        )
...
class ArticleSerializerForAllArticleListView(serializers.ModelSerializer):
    ...
    )  # Article.author (author is ForeignKey with User.) => In Profile model, Profile has one-to-one with User with related name 'profile'
    estimated_reading_time = (
        serializers.ReadOnlyField()
    )  # as estimated_reading_time is property, no need to have any method here
